...
from .qas_ops import QASConv2d, QASLinear, QuantizedReLU
...
def initialize_quantized_model_from_ptq_checkpoint(model: QuantizedMNISTNet, checkpoint: dict) -> None:
    ...
    _copy_buffer_value(model.conv1.w_scale, conv1_w_scale)
    _copy_buffer_value(model.conv1.y_scale, state["conv1.scale"].to(torch.float32))
    _copy_buffer_value(model.relu1.zero_y, state["conv1.zero_point"].to(torch.float32))
    ...
    _copy_buffer_value(model.conv2.w_scale, conv2_w_scale)
    _copy_buffer_value(model.conv2.y_scale, state["conv2.scale"].to(torch.float32))
    _copy_buffer_value(model.relu2.zero_y, state["conv2.zero_point"].to(torch.float32))
    ...
    _copy_buffer_value(model.fc1.w_scale, fc1_w_scale)
    _copy_buffer_value(model.fc1.y_scale, state["fc1.scale"].to(torch.float32))
    _copy_buffer_value(model.relu3.zero_y, state["fc1.zero_point"].to(torch.float32))
    ...
    _copy_buffer_value(model.fc2.w_scale, fc2_w_scale)
